 ...
 *   2·4째주 토요일 진료 · 일요일/공휴일 휴진 · 대체공휴일 정상진료
 *
 * ⚠️ 토요일은 **2·4째주** 진료(오너 확정 2026-09-09, 옛 홈페이지 진료시간 안내 "2, 4째주 토요일 진료합니다" 그대로).
 *    달마다 일정표(MONTHLY_NOTICE)로 다시 확인한다 — 월 일정이 우선.
...
export const HOURS: { lunch: { start: string; end: string }; rows: HourRow[]; display: HourDisplay[]; closed: string } = {
  ...
  rows: [
    ...
    { day: 'Saturday', ko: '토요일', open: '10:00', close: '14:00', note: '2·4째주 진료 · 점심시간 없음', biweekly: true },
  ],
