...
import { Breadcrumb, ContactBand, FaqList, MedicalNotice } from '@/components/ui';
      ...
      <main id="main" className="pt-[72px] md:pt-[84px]">
        <section className="section bg-canvas">
          <div className="wrap">
            ...
            <p className="lead mt-5 max-w-[720px]">진료시간과 예약, 오시는 길, 임플란트·턱관절 치료, 건강보험 적용, 마취와 수면치료까지 환자분들이 가장 많이 물어보시는 내용을 모았습니다.</p>
            <nav aria-label="문답 묶음" className="mt-8 flex flex-wrap gap-2">
              {SITE_FAQ.map((g) => (
                <a key={g.id} href={`#${g.id}`} className="pill hover:border-brand-300 hover:text-brand-700">{g.title}</a>
              ))}
            </nav>
          </div>
        </section>
